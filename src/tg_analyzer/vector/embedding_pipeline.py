...
class EmbeddingPipeline:
    ...
    def _chunk_fixed_size(
        self, text: str, chunk_size: int, metadata: Dict[str, Any]
    ) -> List[TextChunk]:
        ...
        while start_pos < len(text):
            ...
            if end_pos < len(text) and not text[end_pos].isspace():
                ...
                min_chunk_size = int(
                    chunk_size * MIN_WORD_BOUNDARY_RATIO
    ...
    def _chunk_by_sentence(
        self, text: str, max_chunk_size: int, metadata: Dict[str, Any]
    ) -> List[TextChunk]:
    ...
    async def generate_embeddings(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                batch_vectors = await self.llm_manager._get_provider(
                    provider
                ).get_embeddings(batch_texts, model=model)
                all_vectors.extend(batch_vectors)

                # Estimate tokens (rough approximation)
                batch_tokens = sum(
                    len(text) // CHARS_PER_TOKEN_ESTIMATE for text in batch_texts
                )
                ...
                logger.info(
                    f"Processed batch {i // batch_size + 1}: {len(batch_texts)} chunks"
                )
    ...
    async def process_and_store(
        ...
        chunks = self.chunk_text(
            text=text, strategy=chunking_strategy, metadata=source_metadata
        )
        ...
        logger.info(
            f"Pipeline completed: {len(chunks)} chunks, {storage_result['inserted_count']} vectors stored"
        )
        return result
    ...
    async def search_similar(
        ...
        results = await self.vector_client.search_similar(
            query_embedding=query_vector, limit=limit, metadata_filter=metadata_filter
        )
