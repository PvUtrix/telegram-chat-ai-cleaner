...
class ConfigManager:
    ...
    def _find_config_file(self, config_path: Optional[str]) -> Optional[Path]:
        ...
        if env_example.exists():
            logger.warning(
                "Using env.example as config file. Consider copying to .env and setting your values."
            )
            return env_example
    ...
    def validate_config(self) -> Dict[str, str]:
        ...
        if not any(
            ...
            issues["llm"] = (
                "No LLM API key configured. Analysis features will not work."
            )

        if not self._settings.supabase_url or not self._settings.supabase_key:
            issues["vector"] = (
                "Supabase configuration incomplete. Vector features will not work."
            )
        ...
        if self._settings.default_cleaning_approach not in valid_approaches:
            issues["cleaning_approach"] = (
                f"Invalid cleaning approach: {self._settings.default_cleaning_approach}. Must be one of {valid_approaches}"
            )

        if not 1 <= self._settings.default_cleaning_level <= 3:
            issues["cleaning_level"] = (
                f"Invalid cleaning level: {self._settings.default_cleaning_level}. Must be 1, 2, or 3"
            )
