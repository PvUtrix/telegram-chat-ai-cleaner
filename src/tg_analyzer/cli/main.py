...
from ..llm.templates.default_prompts import list_available_templates, get_prompt_template
...
def select_input_file():
    ...
    if not json_files:
        ...
        console.print("Please place your Telegram export JSON files in the data/input/ directory.")
        return None
    ...
    console.print(f"[blue]Found {len(json_files)} JSON files in input directory:[/blue]")
    console.print()
    ...
    console.print()
    console.print("[dim]Select a file by entering its number to process it for cleaning and analysis[/dim]")
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(json_files):
                ...
                console.print(f"[red]Please enter a number between 1 and {len(json_files)}[/red]")
        except click.Abort:
            return None
...
@click.option('--approach', type=click.Choice(['privacy', 'size', 'context']),
              default=None, help='Cleaning approach')
@click.option('--level', type=click.IntRange(1, 3), default=None,
              help='Cleaning level (1-3)')
...
def clean(input_file, approach, level, output_format, output_file, batch, interactive):
    ...
    if interactive:
        ...
        current_approach_num = next((k for k, v in approaches.items() if v == approach), 1)
        current_level_num = level

        console.print(f"[blue]Current approach:[/blue] {current_approach_num}. {approach}")
        ...
        for num, approach_name in approaches.items():
            ...
            if approach_name in example_outputs and 2 in example_outputs[approach_name]:
                ...
                if "\n" in example:
                    console.print(f"     [cyan]Example output (level 2):[/cyan]")
                    for line in example.split("\n"):
                        console.print(f"     [dim]│ {line}[/dim]")
                else:
                    console.print(f"     [cyan]Example output (level 2):[/cyan] [dim]│ {example}[/dim]")
            console.print()  # Add spacing between choices

        while True:
            try:
                approach_input = click.prompt(f"Choose cleaning approach (1-3, default: {current_approach_num})", default=current_approach_num, type=int)
                if approach_input in approaches:
                    approach = approaches[approach_input]
                    break
                else:
                    console.print("[red]Please enter 1, 2, or 3[/red]")
                    console.print("[dim]Example: Enter 1 for privacy, 2 for size, 3 for context[/dim]")
            except click.Abort:
                return

        console.print(f"[blue]Current level:[/blue] {current_level_num}. {levels[current_level_num]}")
        ...
        for num, level_name in levels.items():
            ...
            if approach in example_outputs and num in example_outputs[approach]:
                ...
                # Format multi-line examples properly
                if "\n" in example:
                    console.print(f"     [cyan]Example output:[/cyan]")
                    for line in example.split("\n"):
                        console.print(f"     [dim]│ {line}[/dim]")
                else:
                    console.print(f"     [cyan]Example output:[/cyan] [dim]│ {example}[/dim]")
            console.print()  # Add spacing between choices

        while True:
            try:
                level_input = click.prompt(f"Choose cleaning level (1-3, default: {current_level_num})", default=current_level_num, type=int)
                if level_input in levels:
                    level = level_input
                    break
                else:
                    console.print("[red]Please enter 1, 2, or 3[/red]")
                    console.print("[dim]Example: Enter 1 for basic, 2 for medium, 3 for full[/dim]")
            except click.Abort:
                return
    ...
    if input_path.name.lower() in ['result.json', 'result']:
        ...
        if new_filename:
            ...
            console.print(f"[green]✅ Auto-renamed input file to:[/green] {new_filename}")
        else:
            console.print(f"[dim]File already has a meaningful name, skipping rename[/dim]")
    ...
    if batch:
        ...
        if renamed_files:
            console.print(f"[green]✅ Auto-renamed {len(renamed_files)} generic file(s) before processing[/green]")
    ...
    if all:
        ...
        if not filename:
            ...
            if not json_files:
                ...
                while True:
                    try:
                        choice = click.prompt("Select file to rename (number)", type=int)
                        if 1 <= choice <= len(json_files):
...
@cli.command()
@click.option('--provider', type=click.Choice(['openai', 'anthropic', 'google', 'groq', 'ollama', 'openrouter']),
              help='LLM provider override (default: openrouter)')
@click.option('--model', help='Model override (e.g., openai/gpt-4, anthropic/claude-3-sonnet)')
@click.option('--temperature', type=float, help='Temperature for LLM (0.0-1.0, default: template-specific)')
@click.option('--max-tokens', type=int, help='Maximum tokens for LLM response (default: template-specific)')
def analyze(provider, model, temperature, max_tokens):
    ...
    console.print(f"[bold blue]Step 2: Creating analysis workspace[/bold blue]")
    workspace_info = workspace_manager.create_workspace(cleaned_file)
    console.print(f"[green]✅ Workspace created:[/green] {workspace_info['workspace_name']}")
    ...
    console.print(f"[bold blue]Step 4: Loading chat data[/bold blue]")
    with open(cleaned_file, 'r', encoding='utf-8') as f:
        chat_data = f.read()
    console.print(f"[green]✅ Loaded {len(chat_data)} characters[/green]")
    
    # Step 5: Run analysis scripts
    console.print(f"[bold blue]Step 5: Running {len(selected_templates)} analysis scripts[/bold blue]")
    ...
    for i, template in enumerate(selected_templates, 1):
        ...
        try:
            ...
            result = asyncio.run(script_runner.run_script(
...
def select_cleaned_file(file_manager):
    ...
    for i, file_info in enumerate(cleaned_files, 1):
        console.print(f"{i:2d}. {file_info['relative_path']} ({file_info['size']:.1f} MB)")
    
    console.print()
    console.print("[dim]Select a cleaned file to run AI analysis and extract insights[/dim]")
    console.print("[dim]Example: Enter 1 to select the first file[/dim]")
    while True:
        try:
            choice = click.prompt("Select a file (number)", type=int)
            if 1 <= choice <= len(cleaned_files):
                ...
                console.print(f"[green]Selected:[/green] {selected_file['relative_path']}")
                return selected_file['path']
            else:
                console.print(f"[red]Please enter a number between 1 and {len(cleaned_files)}[/red]")
        except click.Abort:
            return None
...
def select_analysis_templates(template_manager):
    ...
    if not templates:
        console.print("[red]No analysis templates found. Create some templates first.[/red]")
        return []
    ...
    console.print("  - Enter numbers separated by commas (e.g., 1,3,4) - Select specific analysis templates")
    console.print("     Example: 1,3,4 will run templates 1, 3, and 4")
    console.print("  - Enter 'all' to select all templates - Run comprehensive analysis with all available templates")
    ...
    while True:
        try:
            choice = click.prompt("Select templates (e.g., 1,3,4 or 'all')", default="all")
            ...
            if choice.lower() == 'none':
                ...
                if all(1 <= i <= len(templates) for i in indices):
                    ...
                    console.print(f"[red]Please enter numbers between 1 and {len(templates)}[/red]")
...
@cli.command()
@click.argument('input_file', required=False)
@click.option('--provider', type=click.Choice(['openai', 'google', 'ollama']),
              default=None, help='Embedding provider')
@click.option('--model', help='Embedding model')
@click.option('--chunk-strategy', type=click.Choice(['fixed_size', 'sentence', 'paragraph', 'overlap']),
              default='overlap', help='Text chunking strategy')
...
def vectorize(input_file, provider, model, chunk_strategy, chunk_size):
    ...
    if not ctx.config.get('supabase_url') or not ctx.config.get('supabase_key'):
        console.print("[red]Error: Supabase not configured. Please set SUPABASE_URL and SUPABASE_KEY.[/red]")
    ...
    try:
        ...
        console.print(f"[dim]Stored {result.get('vectors_stored', 0)} vectors in database[/dim]")
        console.print(f"[dim]Processing time: {result.get('processing_time', 0):.2f}s[/dim]")
...
def watch(input_dir, interval):
    ...
    try:
        ...
        console.print("[yellow]File watching not yet implemented - use batch processing instead[/yellow]")
...
def web(host, port, reload):
    ...
    try:
        ...
        console.print("[red]Error: uvicorn not installed. Install with: pip install uvicorn[standard][/red]")
        return
